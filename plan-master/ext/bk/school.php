...
class school extends adb{
	...
	function get_jss_schools(){
        $str_query="select applicant_school_id,applicant_school_name,applicant_school_type,applicant_school_category_applicant_school_category_id from applicant_school where applicant_school_type=2";
         if(!$this->sql_query($str_query)){
            $this->log_error(LOG_LEVEL_DB_FAIL, 14, "error while getting school. see {$this->error} for detail");
            return false;
        }
