...
//
// Table class for districts
//
class cdistricts {
	var $TableVar = 'districts';
	...
	function cdistricts() {
		...
		// DistrictID
		$this->DistrictID = new cField('districts', 'districts', 'x_DistrictID', 'DistrictID', '`DistrictID`', 3, -1, FALSE, '`DistrictID`', FALSE);
		$this->DistrictID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['DistrictID'] =& $this->DistrictID;

		// District
		$this->District = new cField('districts', 'districts', 'x_District', 'District', '`District`', 200, -1, FALSE, '`District`', FALSE);
		$this->fields['District'] =& $this->District;

		// RegionID
		$this->RegionID = new cField('districts', 'districts', 'x_RegionID', 'RegionID', '`RegionID`', 3, -1, FALSE, '`RegionID`', FALSE);
		...
		$this->programarea_programarea_id = new cField('districts', 'districts', 'x_programarea_programarea_id', 'programarea_programarea_id', '`programarea_programarea_id`', 19, -1, FALSE, '`programarea_programarea_id`', FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `districts` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		...
		$SQL .= ew_QuotedName('DistrictID') . '=' . ew_QuotedValue($rs['DistrictID'], $this->DistrictID->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->DistrictID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@DistrictID@", ew_AdjustSql($this->DistrictID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("districtsview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "districtsadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("districtsedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("districtsadd.php", $this->UrlParm());
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("districtsdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->DistrictID->CurrentValue)) {
			$sUrl .= "DistrictID=" . urlencode($this->DistrictID->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=districts" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
   ...
   // Common render codes
		...
		$this->DistrictID->CellCssStyle = ""; $this->DistrictID->CellCssClass = "";
		$this->DistrictID->CellAttrs = array(); $this->DistrictID->ViewAttrs = array(); $this->DistrictID->EditAttrs = array();

		// District
		$this->District->CellCssStyle = ""; $this->District->CellCssClass = "";
		...
		$this->DistrictID->CssStyle = "";
		$this->DistrictID->CssClass = "";
