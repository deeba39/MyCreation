...
// Create page object
$community_add = new ccommunity_add();
$Page =& $community_add;

// Page init
$community_add->Page_Init();

// Page main
$community_add->Page_Main();
?>
...
// Create page object
var community_add = new ew_Page("community_add");

// page properties
community_add.PageID = "add"; // page ID
community_add.FormID = "fcommunityadd"; // form ID
var EW_PAGE_ID = community_add.PageID; // for backward compatibility

// extend page with ValidateForm function
community_add.ValidateForm = function(fobj) {
...
community_add.Form_CustomValidate =  
...
community_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
community_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $community->TableCaption() ?><br><br>
<a href="<?php echo $community->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$community_add->ShowMessage();
?>
<form name="fcommunityadd" id="fcommunityadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return community_add.ValidateForm(this);">
<p>
...
<?php if ($community->programarea_programarea_id->getSessionValue() <> "") { ?>
<div<?php echo $community->programarea_programarea_id->ViewAttributes() ?>><?php echo $community->programarea_programarea_id->ViewValue ?></div>
<input type="hidden" id="x_programarea_programarea_id" name="x_programarea_programarea_id" value="<?php echo ew_HtmlEncode($community->programarea_programarea_id->CurrentValue) ?>">
<?php } else { ?>
...
class ccommunity_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $community->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$community->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $community->CurrentAction = "C"; // Copy record
		  } else {
		    $community->CurrentAction = "I"; // Display blank record
		...
		switch ($community->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$community->SendEmail = TRUE; // Send email on add success
		...
		$community->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $community;
		$community->community_districts_DistrictID->CurrentValue = 0;
	}
	...
	function RenderRow() {
		...
		if ($community->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($community->RowType == EW_ROWTYPE_ADD) { // Add row
			...
			if ($community->programarea_programarea_id->getSessionValue() <> "") {
				$community->programarea_programarea_id->CurrentValue = $community->programarea_programarea_id->getSessionValue();
			if (strval($community->programarea_programarea_id->CurrentValue) <> "") {
				$sFilterWrk = "`programarea_id` = " . ew_AdjustSql($community->programarea_programarea_id->CurrentValue) . "";
			$sSqlWrk = "SELECT `programarea_name` FROM `programarea`";
			$sWhereWrk = "";
	...
	function AddRow() {
		...
		// community_districts_DistrictID
		$community->community_districts_DistrictID->SetDbValueDef($rsnew, $community->community_districts_DistrictID->CurrentValue, NULL, TRUE);

		// Call Row Inserting event
		$bInsertRow = $community->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($community->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$community->community_id->setDbValue($conn->Insert_ID());
			$rsnew['community_id'] = $community->community_id->DbValue;

			// Call Row Inserted event
			$community->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
	...
	function SetUpMasterDetail() {
		...
		if (@$_GET[EW_TABLE_SHOW_MASTER] <> "") {
			...
			if ($sMasterTblVar == "programarea") {
				$bValidMaster = TRUE;
				$this->sDbMasterFilter = $community->SqlMasterFilter_programarea();
				$this->sDbDetailFilter = $community->SqlDetailFilter_programarea();
				if (@$_GET["programarea_id"] <> "") {
					$GLOBALS["programarea"]->programarea_id->setQueryStringValue($_GET["programarea_id"]);
					$community->programarea_programarea_id->setQueryStringValue($GLOBALS["programarea"]->programarea_id->QueryStringValue);
					$community->programarea_programarea_id->setSessionValue($community->programarea_programarea_id->QueryStringValue);
		...
		if ($bValidMaster) {
			...
			if ($sMasterTblVar <> "programarea") {
				if ($community->programarea_programarea_id->QueryStringValue == "") $community->programarea_programarea_id->setSessionValue("");
			}
		} else {
