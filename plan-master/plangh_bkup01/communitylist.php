...
if ($community_list->sDbMasterFilter <> "" && $community->getCurrentMasterTable() == "programarea") {
...
class ccommunity_list {
	...
	function Page_Main() {
		...
		// Load master record
		if ($community->getMasterFilter() <> "" && $community->getCurrentMasterTable() == "programarea") {
			global $programarea;
	...
	function ResetCmd() {
		...
		if (@$_GET["cmd"] <> "") {
			...
			if (strtolower($sCmd) == "resetall") {
				...
				$community->programarea_programarea_id->setSessionValue("");
			}
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList('schools')) {
			...
			$oListOpt->Body = "<a href=\"schoolslist.php?" . EW_TABLE_SHOW_MASTER . "=community&community_id=" . urlencode(strval($community->community_id->CurrentValue)) . "\">" . $oListOpt->Body . "</a>";
		}
