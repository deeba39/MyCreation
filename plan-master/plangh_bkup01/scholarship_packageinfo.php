...
class cscholarship_package {
	...
	function cscholarship_package() {
		...
		// start_date
		$this->start_date = new cField('scholarship_package', 'scholarship_package', 'x_start_date', 'start_date', '`start_date`', 135, 7, FALSE, '`start_date`', FALSE);
		$this->start_date->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['start_date'] =& $this->start_date;

		// end_date
		$this->end_date = new cField('scholarship_package', 'scholarship_package', 'x_end_date', 'end_date', '`end_date`', 135, 7, FALSE, '`end_date`', FALSE);
	...
	function SqlDetailFilter_grant_package() {
		return "`grant_package_grant_package_id`=@grant_package_grant_package_id@";
	}
	...
	function ApplyUserIDFilters($sFilter) {
		...
		if (!$this->AllowAnonymousUser() && $Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			...
			if ($this->getCurrentMasterTable() == "grant_package")
				$sFilter = $this->AddDetailUserIDFilter($sFilter, "grant_package"); // Add detail User ID filter
