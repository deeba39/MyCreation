...
<input type="text" name="x_towncity" id="x_towncity" title="<?php echo $schools->towncity->FldTitle() ?>" size="30" maxlength="45" value="<?php echo $schools->towncity->EditValue ?>"<?php echo $schools->towncity->EditAttributes() ?>>
	...
	<tr<?php echo $schools->RowAttributes() ?>>
		...
		<td<?php echo $schools->community_community_id->CellAttributes() ?>><span class="ewSearchOpr"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_community_community_id" id="z_community_community_id" value="="></span></td>
...
if (is_array($schools->community_community_id->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($schools->community_community_id->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
function BuildAdvancedSearch() {
	...
	$this->BuildSearchUrl($sSrchUrl, $schools->community_community_id); // community_community_id
...
function ConvertSearchValue(&$Fld, $FldVal) {
	...
	function LoadSearchValues() {
		...
		$schools->community_community_id->AdvancedSearch->SearchValue = ew_StripSlashes($objForm->GetValue("x_community_community_id"));
		$schools->community_community_id->AdvancedSearch->SearchOperator = $objForm->GetValue("z_community_community_id");
