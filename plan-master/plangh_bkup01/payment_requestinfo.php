...
//
// Table class for payment_request
//
class cpayment_request {
	var $TableVar = 'payment_request';
	...
	//
	function cpayment_request() {
		global $Language;

		// payment_request_id
		$this->payment_request_id = new cField('payment_request', 'payment_request', 'x_payment_request_id', 'payment_request_id', '`payment_request_id`', 3, -1, FALSE, '`payment_request_id`', FALSE);
		...
		// year
		$this->year = new cField('payment_request', 'payment_request', 'x_year', 'year', '`year`', 18, -1, FALSE, '`year`', FALSE);
		$this->year->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['year'] =& $this->year;

		// request_date
		$this->request_date = new cField('payment_request', 'payment_request', 'x_request_date', 'request_date', '`request_date`', 133, 7, FALSE, '`request_date`', FALSE);
		...
		// programarea_id
		$this->programarea_id = new cField('payment_request', 'payment_request', 'x_programarea_id', 'programarea_id', '`programarea_id`', 3, -1, FALSE, '`programarea_id`', FALSE);
		$this->programarea_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['programarea_id'] =& $this->programarea_id;

		// request_status
		$this->request_status = new cField('payment_request', 'payment_request', 'x_request_status', 'request_status', '`request_status`', 202, -1, FALSE, '`request_status`', FALSE);
		$this->fields['request_status'] =& $this->request_status;

		// code
		$this->code = new cField('payment_request', 'payment_request', 'x_code', 'code', '`code`', 200, -1, FALSE, '`code`', FALSE);
		$this->fields['code'] =& $this->code;

		// financial_year_financial_year_id
		$this->financial_year_financial_year_id = new cField('payment_request', 'payment_request', 'x_financial_year_financial_year_id', 'financial_year_financial_year_id', '`financial_year_financial_year_id`', 3, -1, FALSE, '`financial_year_financial_year_id`', FALSE);
		...
		// amount
		$this->amount = new cField('payment_request', 'payment_request', 'x_amount', 'amount', '`amount`', 3, -1, FALSE, '`amount`', FALSE);
		$this->amount->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['amount'] =& $this->amount;

		// group_id
		$this->group_id = new cField('payment_request', 'payment_request', 'x_group_id', 'group_id', '`group_id`', 3, -1, FALSE, '`group_id`', FALSE);
	...
	function HighlightName() {
		return "payment_request_Highlight";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "payment_requestlist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("payment_requestview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "payment_requestadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("payment_requestedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("payment_requestadd.php", $this->UrlParm());
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("payment_requestdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=payment_request" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
...
function Row_Inserting(&$rs) {
    ...
    $rs["year"]=date("Y");
    $rs["request_date"]=date("Y-m-d");
    $rs["amount"]=0.00;
    $rs["request_status"]="NEWREQ"; 
    return TRUE;
}
