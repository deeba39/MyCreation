...
class csponsored_student {
   ...
   // Common render codes
		...
		// student_applicant_student_applicant_id
		$this->student_applicant_student_applicant_id->CellCssStyle = ""; $this->student_applicant_student_applicant_id->CellCssClass = "";
	...
	function Row_Updating(&$rsold, &$rsnew) {
		...
		// Enter your code here
		// To cancel, set return value to FALSE
                echo $_SESSION[EW_PROJECT_NAME]["PROGRAM_AREA"];
                
                if($_SESSION[EW_PROJECT_NAME]["PROGRAM_AREA"]!=0)
                {
                    if($rsold['student_resident_programarea_id']->CurrentValue!=$_SESSION[EW_PROJECT_NAME]["PROGRAMAREA"])
                    {
                        return FALSE;
                    }
                }
