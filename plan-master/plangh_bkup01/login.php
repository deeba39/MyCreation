...
function User_LoggedIn($usr) {
    //echo "User Logged In";
    if($_SESSION[EW_PROJECT_NAME . "_status_UserID"]==-1){
		$_SESSION[EW_PROJECT_NAME]["PROGRAM_AREA"]=0;
		//$_SESSION[EW_PROJECT_NAME]["PROGRAM_AREA_NAME"]="Bawjiase";
	}else{
		include_once("ext/programarea.php");
		$p=new programareas();
		$programarea_id=$p->get_user_program_area($usr);
		$row=$p->get_programarea($programarea_id);
		$programarea_name=$row["programarea_name"];
		echo "p=$programarea_id";
		if(programarea_id===false){
			echo "The users program area could not be loaded. Please check with this sytem's administrator.";
			exit();
		}
		$_SESSION[EW_PROJECT_NAME]["PROGRAM_AREA"]=$programarea_id;
		$_SESSION[EW_PROJECT_NAME]["PROGRAM_AREA_NAME"]=$programarea_name;
	}
